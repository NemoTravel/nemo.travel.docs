...
#### Как настроить
* Перейдите в раздел /settings__nc_notifications;
* Включите настройку enable_notify;
* В поле notification_url укажите адрес для отправки нотификаций (например, https://test.graphql.mlsd.ru/notifications/accept);
* В настройке format выберите формат отправки нотификаций (AviaAPI или NDC).
...
- **BookingId** - ID бронирования, для которой требуется отправить нотификацию;
- **Supplier** - наименование поставщика услуг;
- **PnrLocator** - идентификатор заказа в системе поставщика услуг;
- **EventId** - уникальный идентификатор события;
- **TriggerType** - признак об источнике нотификаций:
    -   **ClientApiCall** — клиентский запрос;
    -   **SupplierCallback** — колбэк от поставщика.
