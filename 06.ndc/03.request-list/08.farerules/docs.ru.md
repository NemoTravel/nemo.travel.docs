...
##### Описание
Запрос FareRules позволяет получить правила, применяемые к тарифам определённого перелёта в текстовой форме, для ознакомления или для иных целей.
Операция выполняется успешно, если соблюдаются условия:
-	Задан ID пакета реквизитов (источник), в котором был найден перелёт.
