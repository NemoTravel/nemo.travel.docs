...
-   **Flight.ID** — идентификатор перелёта. Тип данных — строка. При изменении формата Flight.ID был введен разделитель " | " - знак вертикальной черты, и теперь Flight.ID будет иметь вид: ID события|порядковый_номер_лога_в_рамках_ивента|номер_перелёта_по_порядку (пример - 1234567812|7|0)
...
-   **Flight.PriceInfo.Price.TimeLimit** — таймлимит данной цены в зоне UTC+0 (цена действительная до) в формате ISO 8601: <code>yyyy-mm-ddthh:mm:ssZ</code>. Тип данных — ZonedDateTime.
