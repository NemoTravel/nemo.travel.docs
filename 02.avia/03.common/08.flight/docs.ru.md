...
-   **Flight.ID** — идентификатор перелёта. Тип данных — строка. При изменении формата Flight.ID был введен разделитель, и теперь Flight.ID будет иметь вид: ID события|порядковый_номер_лога_в_рамках_ивента|номер_перелёта_по_порядку (пример - 1234567812|7|0)
