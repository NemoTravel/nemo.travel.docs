...
-   **Flight** — корневой элемент, внутри которого находятся данные перелёта. Тип данных — массив.
...
-   **Flight.TypeInfo** — типизация перелёта по различным критериям. Тип данных — массив.
...
<!-- он сейчас используется во внутренних процессах, при mOW поисках, наружу сейчас получается не выдаётся
Вероятно будет задействован когда сделаем новую выдачу с отдельными mOW плечами   **Flight.TypeInfo.MultyOWLeg** — признак что данный перелёт является плечом мультиOW перелёта. Тип данных — булевский. -->
...
<!-- Устаревший функционал **hRT** — RT/2 — запрашивался простой OW перелёт, но на основании настроек определённого пакета реквизитов был запущен RT/2 поиск; -->
...
-   **Flight.Segments** — контейнер для сегментов перелёта. Тип данных — массив.
-   **Flight.Segments.Segment** — информация о сегменте перелёта. Тип данных — массив.
-   **Segment.ID** — порядковый номер данного сегмента в перелёте. Тип данных — целое 32-битное число.
-   **Segment.RequestedSegment** - номер сегмента перелёта из запроса поиска. Тип данных - целое 32 битное число.
-   **Segment.DepAirp** — информация об аэропорте отправления для данного сегмента. Тип данных — массив.
...
-   **Segment.ArrAirp** — информация об аэропорте прибытия для данного сегмента. Тип данных — массив. Формат аналогичен аэропорту отправления
-   **Segment.ETicket** — признак возможности выписки электронного билета на данном сегменте. Тип данных — булевский.
-   **Segment.StopPoints** — список точек остановок на данном сегменте перелёта. Тип данных — массив.
-   **Segment.StopPoints.StopPoint** — информация об одной из точек остановок на данном сегменте перелёта. Тип данных — массив.
...
-   **Segment.BookingClass** — информация о классе перелёта для данного сегмента перелёта. Тип данных — массив.
...
-   **Segment.NotAirplaneSegmentInd** — признак (индикатор) наземного сегмента. Тип данных — булевый.
...
-   **Flight.PriceInfo** — информация о ценах для данного перелёта. Тип данных — массив.
-   **Flight.PriceInfo.Price** — информация о конкретной цене для данного перелёта. Тип данных — массив.
...
-   **Flight.PriceInfo.Price.PassengerFares** — массив ценовых составляющих по типам пассажиров. Тип данных — массив.
-   **Flight.PriceInfo.Price.PassengerFares.PassengerFare** — ценовая составляющая для конкретного типа пассажира. Тип данных — массив.
...
-   **PassengerFare.Taxes** — контейнер для такс данной ценовой составляющей. Тип данных — массив.
-   **PassengerFare.Taxes.Tax** — информация о конкретной таксе. Тип данных — массив.
...
-   **PassengerFare.Tariffs** — контейнер для тарифов данной ценовой составляющей. Тип данных — массив.
-   **PassengerFare.Tariffs.Tariff** — информация об одном из тарифов данной ценовой составляющей. Тип данных — массив.
...
-   **PassengerFare.Tariffs.Tariff.FreeBaggage** — содержит информацию о бесплатном багаже по данному тарифу. Тип данных — массив.
...
-   **PassengerFare.Commission** — информация о комиссии для данной ценовой составляющей от GDS. Тип данных — массив.
...
-   **PassengerFare.ExchangePriceInfo** — общая плата за обмен (элемент будет только при получении вариантов обмена). Тип данных — массив
-   **PassengerFare.ExchangePriceInfo.AirlinePenalty** — штраф авиакомпании за обмен. Тип данных — массив.
...
-   **PassengerFare.ChargeBreakdown.Charge** - информация о конкретном сборе, величине округления. Тип данных — массив.
...
-   **Flight.Price.PricingData** — результаты расчета ценообразования. Тип данных — массив.
