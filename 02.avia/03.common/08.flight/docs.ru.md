...
-   **Flight.ID** — идентификатор перелёта. Тип данных — целое 128-битное число.
...
-   **Flight.TypeInfo.DirectionType** — тип маршрута перелёта. Тип данных — перечисление, возможные значения:
    ...
    -   **SingleOJ** — одинарный Open Jaw — перелёт из 2-х плечей, у которого точка вылета первого плеча совпадает с точкой прилёта второго сегмента ИЛИ точка прилёта первого плеча совпадает с точкой вылета второго плеча;
