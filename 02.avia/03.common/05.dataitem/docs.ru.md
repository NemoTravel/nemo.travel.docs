...
-   **TimeLimits** — таймлимиты (необязательный). Тип данных — сложный.
...
-   **FOPInfo** — форма оплаты для передачи в систему поставщика услуги (необязательный). Тип данных — сложный.
...
-   **SourceInfo** — информация об источнике, где была создана бронь услуги (необязательный). Тип данных — сложный.
...
-   **Document** — документ, удостоверяющий личность путешественника (необязательный). Тип данных — сложный.
-   **Document.Type** — тип документа. Тип данных — перечисление, возможные значения описаны в [Типы документов](http://support.nemo.travel/ru/%D0%A2%D0%B8%D0%BF%D1%8B_%D0%B4%D0%BE%D0%BA%D1%83%D0%BC%D0%B5%D0%BD%D1%82%D0%BE%D0%B2).
...
-   **ContactInfo** — контактные данные (при бронировании в Farelogix и при использовании SirenaAviaPlus указание контактных данных обязательно). Тип данных — сложный.
...
-   **LoyaltyCard** — карточка лояльности (необязательный). Тип данных — сложный.
...
-   **ElectronicDocument** — электронный документ (билет, EMD) (необязательный). Тип данных — сложный.
...
-   **PaperDocument** — бумажный документ (необязательный). Тип данных — сложный.
...
-   **Endorsements** — эндорсменты (необязательный). Тип данных — сложный.
...
-   **Visa** — данные визы (необязательный). Тип данных — сложный.
...
-   **ArrivalAddress** — адрес пребывания (необязательный). Тип данных — сложный.
...
-   **BookedSeat** — данные бронируемого или забронированного места из карты мест (необязательный). Тип данных — сложный.
...
-   **ValidatingCompany** — информация о валидирующем перевозчике (необязательный). Тип данных — сложный.
...
-   **TourCode** — туркод (необязательный). Тип данных — сложный.
...
-   **Discount** — скидка от тарифа (необязательный). Тип данных — сложный.
...
-   **FareSourceCode** -  Информация об уникальном коде тарифа (Специфика Mystifly) (необязательный). Тип данных — сложный.
-   **FareSourceCode.Code** - код тарифа, определяющий перелёт в системе Mystifly. Тип данных — строка.
-   **AdditionalLocators** - используется в uAPI и NemoNetwork для отображение локатора, по которому можно прочитать ПНР в терминале ГРС (необязательный). Тип данных — сложный.
...
-   **OSI** (Other Service Information) — дополнительная информация, отправляемая в авиакомпанию (необязательный). Тип данных — сложный.
...
-   **DiscountDocument** — документ пассажира, являющийся основанием для субсидии или скидки (необязательный). Тип данных — сложный.
...
-   **CashValueForMultiFOPProxing** — содержит значение суммы для cash части при мульти-FOP GDS-процессинге через сторонние платежные шлюзы (необязательный). Тип данных — сложный.
...
-   **PNRFOP** — содержит формы оплаты в брони (необязательный). Тип данных — сложный. Используется только в ответах для передачи реально проставленной формы оплаты в PNR.
...
-   **Markup** — информация о сборе агента (необязательный). Тип данных — сложный.
...
-   **TicketingProxy** — данные для проксирования выписки через платежный шлюз (необязательный). Тип данных — сложный.
...
-   **CRMIntegration** — данные для прописывания в PNR для интеграции с CRM или BO системами, работающими с системами поставщиков (необязательный). Тип данных — сложный.
...
-   **CRMIntegration.SalesChannel** — канал продажи. Тип данных — перечисление, допустимые значение:
    ...
    -   **API**;
    -   **AgentSite**.
-   **EndUserData** — данные конечного пользователя системы (необязательный). Тип данных — сложный.
