...
>>>> В случае наличия статуса NoAirlineLocator выписка невозможна. Чтобы выписать заказ, нужно повторить запрос [UpdateBook](/avia/request/updatebook) несколько раз, пока этот статус не пропадет.
