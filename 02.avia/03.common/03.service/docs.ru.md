...
-   **SubStatus** - содержит информацию о подстатусе.
	-   **SubStatus.Status** — подстатус, уточняющий текущее состояние услуги. Тип данных — перечисление, возможные значения:
        ...
        -   **SupplierDidNotFoundOrder** - от поставщщика вернулась ошибка Order not found связанная с временными проблемами на стороне поставщика, в данный момент актуально только для поставщика авиа контента S7NDC.
