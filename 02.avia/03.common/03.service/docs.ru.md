...
>>>> В случае наличия этого статуса выписка невозможна. Чтобы выписать заказ, нужно повторить запрос [UpdateBook](/avia/request/updatebook) несколько раз, пока этот статус не пропадет.
