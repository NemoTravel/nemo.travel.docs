...
-   **RequestedFlightInfo.ODPair.DepaturePoint.IsCity** — признак того, что в качестве точки отправления указан код города (агрегатора аэропортов) (необязательный). В случае, если код города и аэропорта совпадают, в приоритете берется аэропорт. Тип данных — булевский.
