...
-   **RequestedFlightInfo.ODPair.DepaturePoint.IsCity** — Признак того, что переданный код является кодом города (Необязательный). Если не передаётся в запрос, то срабатывает внутренняя логика определения данного признака на основе справочника. Тип данных — булевский.
