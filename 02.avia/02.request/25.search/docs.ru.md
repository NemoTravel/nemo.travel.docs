...
-   **Passengers.Passenger.PricedAs** — категория пассажира для переопределения (необязательное поле). Тип данных - строка.
...
-   **Restrictions.SubscriptionSearch** - идентификатор поиска с подписками. Тип данных - bool.
...
-   **SearchData.IsComplete** — признак завершённости поиска. Тип данных — булевский.
