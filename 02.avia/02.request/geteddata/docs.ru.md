...
Используется для получения маски электронного документа из ГРС. На текущий момент поддерживается получение маски для броней из ГРС Sirena.
...
-   **ExchangedTicketNumber** - номер старого билета, по которому был выполенен обмен. Тип данных — строка. 
...
-	**Service.FlightService** - содержит описание услуги по авиаперелета. Тип данных - [Flight](/avia/common/flight).
