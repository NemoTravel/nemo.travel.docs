...
-   **Passengers.BookedPerson.Ticket.RefundSum** - Сумма к возврату от стоимости билета.  Тип данных - сложный. (Данный параметр является устаревшим и скоро будет удалён, в замен ему уже используется параметр RefundPrice).
...
-   **Status** - статус заказа. Тип данных - перечисление. Возможные значения:
    ...
    -   **cancel (1)** - Отменён
    -   **ticket (2)** - Выписан 
-   **StatusChanging** - признак того, что статус брони точно не известен и мог измениться, т.к. в процессе операции возникла ошибка. При значении true необходимо обновить бронь. Тип данных - булев.
-   **SupplierStatus** - статус на стороне поставщика. Только для ГРС Сирена. Тип данных - строка.
-   **TimelimitToConfirm** - таймлимит на подтверждения бронирования. актуально для поставщиков УЖД (УИТ), Сирена, УФС. Тип данных - строка.
