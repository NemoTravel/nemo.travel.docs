...
Ниже будет описано поле, которое встречается только в ответе ConfirmBook и отсутствует в BookTrain.

-   **Passengers.BookedPerson.Tickets.Ticket.RefundTimeLimit** - таймлимит на возврат. Меняет свое значение в зависимости от наличия ЭР (ее присутствии/отсутствии). Тип данных - строка. У этого поля есть 4 возможных линии поведения (актуально для поставщиков: УФС и Сирена):
	- ЭР отсутствует и международное направление (directiongroup 1 и 2): не менее, чем за 6 часов до отправления поезда со станции посадки пассажира, т.е. смотрим на значение OnlineTicketReturnExpiration от поставщика, из него вычитаем 6 часов и получаем значение, которое должно быть в RefundTimeLimit
	- ЭР отсутствует и внутрироссийское направление (directiongroup 0 и 3): возврат должен быть доступен до отправления поезда
	- ЭР присутствует, OnlineTicketReturnExpiration от поставщика вернулся не пустым: парсим значение от поставщика
	- ЭР присутствует, OnlineTicketReturnExpiration вернулся пустым: ограничиваем за 1 час до отправления поезда

Для остальных ЖД поставщиков (КТЖ, УЖД, УИТ) для обоих типов маршрута (OW и RT) значение OnlineTicketReturnExpiration ограничиваем одним часом до отправления поезда со станции посадки пассажира.
