...
-   **SeatsPref.LocPref** - описание. Тип данных - перечисление. Возможные значения (может быть null):
...
-   **SeatsPref.GenderPref** - Гендерный тип бронирования. Тип данных - перечисление. Возможные значения аналогичны параметру TCategory.GenderType из ответа на поиск.
...
-   **BookRQPerson** - Пассажир бронирования. Тип данных - сложный (Содержит все свойства элемента Person из общих элементов + дополнительное свойство).
-   **BookRQPerson.NeedServices** - Доп. Тип данных - перечисление. Возможные значения аналогичны параметру Car.Services из ответа на поиск поездов.
-   **BookRQPerson.ReturnTrainNeedServices** - Доп для поезда обратно, если оформляется ЖД перевозка типа "туда-обратно". Аналогичен параметру BookRQPerson.NeedServices.
-   **BookRQPerson.RzhdBBonusCard** - Карта РЖД Бонус для начисления баллов. Тип данных - строка.
-   **BookRQPerson.RzhdBDiscountCard** - Карта РЖД Бонус для получения скидки. Тип данных - строка.
-   **BookRQPerson.DiscountCard** - Дисконтная(универсальная) карта для получения скидки. Тип данных - строка.
...
-   **BookedPerson** - описание. Тип данных - сложный (Содержит все свойства элемента Person из общих элементов + дополнительные свойства).
-   **BookedPerson.Price** - Стоимость всех билетов пассажира. Тип данных - сложный. Структура аналогична параметру TCategory.Price из ответа на запрос поиска.
-   **BookedPerson.Tickets** - Билеты пассажира. Тип данных - массив элементов TicketInformation.
-   **Ticket** - Информация о билете. Тип данных - сложный.
-   **Ticket.TicketNumber** - Номер билета. Тип данных - строка.
-   **Ticket.IsERegistered** - Статус электронной регистрации (ЭР) билета. true - включена ЭР, false - ЭР выключена. Тип данных - булев.
-   **Ticket.BlankID** - Идентификатор бланка в системе поставщика. Тип данных - строка.
-   **Ticket.Status** - Статус билета. Может отличаться от общего статуса заказа. Тип данных - перечисление. Возможные значения аналогичны параметру Status.
-   **Ticket.IsPrinted** - Признак распечатки билета. Тип данных - булев.
-   **Ticket.TariffType** - Тип тарифа, ПОЛНЫЙ и тп. - описание из ГДС. Тип данных - строка.
-   **Ticket.RefundCode** - Ид транзакции возврата. Тип данных - строка.
-   **Ticket.SeatNum** - Номер места в вагоне. Тип данных - строка.
-   **Ticket.Services** - Дополнительные услуги. Тип данных - перечисление. Возможные значения аналогичны параметру Car.Services из ответа на запрос поиска (может быть несколько через пробел)(может быть пустым).
-   **Ticket.Price** - Стоимость билета. Тип данных - сложный. Структура аналогична параметру TCategory.Price из ответа на запрос поиска.
-   **Ticket.Service** - Стоимость сервиса по электронному билету. Доступно только при бронировании через UFS. Тип данных - сложный. Структура аналогична параметру   TCategory.Price из ответа на запрос поиска.
-   **Ticket.RefundPrice** - Сумма к возврату от стоимости билета. Доступно только при бронировании через UFS и после получения дополнительной информации перед сдачей билетов. Тип данных - сложный. Структура аналогична параметру TCategory.Price из ответа на запрос поиска.
-   **Ticket.RefundService** - Сумма к возврату от стоимости сервиса по электронному билету. Доступно только при бронировании через UFS и после получения дополнительной информации перед сдачей билетов. Тип данных - сложный. Структура аналогична параметру TCategory.Price из ответа на запрос поиска.
-   **Ticket.RefundPenalty - Сумма к возврату от суммы штрафов. Доступно только при бронировании через UFS и после получения дополнительной информации перед сдачей билетов. Тип данных - сложный. Структура аналогична параметру TCategory.Price из ответа на запрос поиска.
-   **Ticket.IsReturn** - Является ли билет обратным. Тип данных - булев.
-   **Ticket.TransportDocs** - Транспортировочные документы. Тип данных - массив элементов TransportDoc.
...
-   **TransportDoc.Price** - Стоимость документа на багаж. Тип данных - сложный. Структура аналогична параметру TCategory.Price из ответа на запрос поиска.
-   **TransportDoc.RefundCode** - Код возврата. Тип данных - строка.
-   **TransportDoc.OrderNumber** - Номер заказа. Тип данных - строка.
-   **Train** - Поезд, в котором бронируются билеты. Тип данных - сложный. Структура аналогична параметру Train из ответа на запрос поиска.
-   **ReturnTrain** - Поезд, в котором бронируются обратные билеты. Тип данных - сложный. Структура аналогична параметру Train из ответа на запрос поиска.
-   **Price** - Полная стоимость брони. Тип данных - сложный. Структура аналогична параметру TCategory.Price из ответа на запрос поиска.
-   **WasSuccessTicketing** - описание. Тип данных - булев.
